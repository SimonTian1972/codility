...
void sgemm_vec(size_t size_m, size_t size_n, size_t size_k,
    ...
    for (size_t i = 0; i < size_m; ++i) {
        size_t vl;
        size_t n = size_n;
        for (size_t j = 0; j < size_n; j = j + vl, n = n - vl) {
            vl = __riscv_vsetvl_e32m8(n);
            vfloat32m8_t vec_sum = __riscv_vle32_v_f32m8(c + i * ldc + j, vl);

            for (size_t k = 0; k < size_k; ++k) {
                float ik = a[i * lda + k];
                vfloat32m8_t vec_kj = __riscv_vle32_v_f32m8(b + k * ldb + j, vl);
                vec_sum = __riscv_vfmacc_vf_f32m8(vec_sum, ik, vec_kj, vl);
            }
            printf("here j= %zu vl= %zu n= %zu  \n", j, vl, n);
            __riscv_vse32_v_f32m8(c + i * ldc + j, vec_sum, vl);
        }
    }
}
